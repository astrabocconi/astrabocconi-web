...
| `brand-scoller.tsx` | later brief | The partner marquee in the second blue bar. Filename keeps the brief's spelling so later snippets importing it line up. Two changes: the four `react-icons` brand glyphs became a prop-driven list with blank plates, since ASTRA's own logos are coming and those icons would be thrown away; and `[gap:var(--gap))]` had a stray bracket, which Tailwind drops, so the gap never applied and the loop seam showed. Keyframes are in globals.css. |
| `expand-on-hover.tsx` | later brief | Six panels that expand under the cursor, standing in for the chi siamo visual. Two deliberate deviations: it imported `framer-motion` when this project already ships `motion`, the same library renamed, so it imports `motion/react` instead of installing a duplicate; and it imported four `swiper/css` stylesheets while using no Swiper at all, so those went rather than pull in a package for stylesheets nothing reads. Panels are blank white, the overlay gradient is ASTRA blue, and the sizes are props so it fits a half width column. |
